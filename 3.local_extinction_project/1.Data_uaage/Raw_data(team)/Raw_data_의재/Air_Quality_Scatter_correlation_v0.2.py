...
for city_name in city_names:
    for air_name in air_names:        
        file_path = f"{base_dir}/{air_name}_도시별_Analytics_Data/{air_name}_correlation_collection_last(utf-8).csv"

        df_correlation = pd.read_csv(file_path)
        
        df_correlation.plot(kind='scatter', x='P-Value', y='Correlation', c='coral', s=150, figsize=(10, 7), marker ='o')
        plt.title(f'전국 {air_name}-인구밀도 상관관계 산점도',fontsize='22')
        labels = [
            '서울', '부산', '대구', '인천', '광주', 
            '대전', '울산', '세종', '경기도', '강원도', 
        ...
        label_x = df_correlation['P-Value'].tolist()
        label_y = df_correlation['Correlation'].tolist()

        #plt.ylim(-1,2)
        #plt.xlim(-1,2)
        plt.xlabel('P-Value',fontsize='18')
        plt.ylabel('상관관계',fontsize='19')
        plt.xticks(fontsize=15)
        plt.yticks(fontsize=15)

        for i, txt in enumerate(labels):
            plt.annotate(txt, (label_x[i], label_y[i]), textcoords='offset points', xytext=(10, -1),fontsize=16)


        #폴더생성후 따로 실행 해줘야함
        plt.savefig(f"{base_dir}/{air_name}_도시별_Analytics_Data/{air_name}_상관관계_산점도.png")
