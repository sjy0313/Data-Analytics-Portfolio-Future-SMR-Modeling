...
for city_name in city_names:
    for air_name in air_names:
        ...
        if correlation_value > 0:
            ...
            result_df.to_csv(output_file_path_utf_8, index=False, encoding='utf-8')
            result_df.to_csv(output_file_path_euc_kr, index=False, encoding='euc-kr')
            result_df.to_csv(output_file_corr_path_utf_8, index=False, encoding='utf-8')
            result_df.to_csv(output_file_corr_path_euc_kr, index=False, encoding='euc-kr')
...
correlation_matrix = df_correlation_drop[['인구밀도', f'{air_name}']].corr()
...
for city_name in city_names:
    for air_name in air_names:
        ...
        correlation_value = df.loc[df['Metric'] == 'Correlation', 'Value'].values[0]
