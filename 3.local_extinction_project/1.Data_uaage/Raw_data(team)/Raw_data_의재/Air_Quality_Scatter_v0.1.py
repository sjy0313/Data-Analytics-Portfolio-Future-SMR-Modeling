...
Some_data_air = df_air[df_air['구분'] == '부산광역시']
...
plt.scatter(Some_data_air['구분'], Some_data_population['행정구역(시군구)별'], color='blue')

# 각 점에 라벨을 추가합니다.
for i in range(len(df)):
    plt.text(Some_data_air['구분'][i], Some_data_population['행정구역(시군구)별'][i], df['City'][i], fontsize=9)
